...
# api.add_resource(Profile_api,'/api/profile/<string:user_id>')
api.init_app(app)
    ...
    if request.method == 'POST':
        ...
        if data:
            if str(data.password) == password:
                ...
                # print(following)
                return render_template('feed.html', icon=profile, name=name, posts=data, following=following)
            else:
        ...
        if current_user.is_authenticated:
            data = requests.get(request.root_url+'api/posts/'+'0')
            ...
            return render_template('feed.html', icon=profile, name=name, posts=data.json(), following=following)
...
def create():
    if request.method == "POST":
        # path='..\\static\\profiles\\'+request.form['user_id']+'.jpg'
        # path=os.path.abspath('profiles')
        ...
        if requests.post(url=request.url_root+'api/accounts', json=form).status_code == 201:
            ...
            return render_template('users.html', users=users, current_user=current_user.user_id)
        else:
...
def post():
    if request.method == 'POST':
        ...
        # print(stamp)
        # print(type(stamp))
        # path=os.path.abspath(+current_user.user_id)
...
# @app.route('/other', defaults={'var': 'post'}, methods=["GET", "POST"])
@app.route('/other_profile/<string:var>,<string:user_id>,<string:other>', methods=["GET", "POST"])
@login_required
def other_profile(var,user_id,other):
    other_user=requests.get(url=request.root_url+'api/accounts/'+user_id+'/False/False').json()
    posts = requests.get(url=request.url_root +
                         'api/posts/'+user_id)
    follower = requests.get(url=request.url_root +
...
def search():
    if request.method == "GET":
        ...
        print(following)
        # print(users.json())
        return render_template('users.html', users=users.json(), current_user=current_user.user_id, following=following)
...
@app.route('/unlike/<string:post_id>')
@login_required
def unlike(post_id):
